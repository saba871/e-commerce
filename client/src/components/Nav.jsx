        ...
        <header className="reveal sticky top-4 z-50 mx-auto w-[95%] max-w-7xl">
            <div className="glass-panel flex items-center justify-between !py-4 !px-8 relative">
                ...
                <div className="flex items-center gap-4 md:gap-6 z-[60]">
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        {user ? (
                            <>
                                ...
                                <button onClick={logout} className="text-xs font-bold text-red-500 hover:underline uppercase tracking-widest">
                                    Exit
                                </button>
                            </>
                        ) : (
                    ...
                    <button onClick={() => setIsOpen(!isOpen)} className="p-2 md:hidden text-slate-900 transition-colors">
                        {isOpen ? <X size={24} /> : <Menu size={24} />}
                    </button>
                </div>

                {/* Mobile Menu Overlay */}
                <div className={`fixed inset-0 top-0 left-0 w-full h-screen bg-white/95 backdrop-blur-xl transition-all duration-500 md:hidden z-[50] ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full pointer-events-none"}`}>
                    <div className="flex flex-col items-center justify-center h-full gap-8 text-center">
                        {navLinks.map((link) => (
                            <NavLink key={link.to} to={link.to} onClick={() => setIsOpen(false)} className="text-3xl font-bold text-slate-900 hover:text-slate-500 transition-colors">
                                {link.label}
                            </NavLink>
                        ))}
                        ...
                        {user ? (
                            <div className="flex flex-col gap-6">
                                <NavLink to="/panel" onClick={() => setIsOpen(false)} className="text-2xl font-medium">
                                    Studio Panel
                                </NavLink>
                                <button
                                    onClick={() => {
                            ...
                            <Link to="/login" onClick={() => setIsOpen(false)} className="primary-btn !text-lg !py-4 !px-10">
                                Access Studio
                            </Link>
                        )}
                    </div>
