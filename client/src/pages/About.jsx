...
const About = () => {
	return (
		<motion.main initial="initial" animate="animate" className="mx-auto w-full max-w-[1600px] space-y-32 pb-24 pt-12 px-6 sm:px-10 lg:px-20">
			...
			<section className="relative h-[60vh] min-h-[500px] w-full overflow-hidden rounded-[3rem] shadow-2xl">
				<motion.div initial={{ scale: 1.1, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.5 }} className="absolute inset-0">
					<img src="https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=1800&q=80" alt="Studio" className="h-full w-full object-cover" />
			...
			<section className="grid gap-12 lg:grid-cols-12">
				...
				<motion.div whileInView="animate" viewport={{ once: true }} variants={fadeInUp} className="glass-panel space-y-8 lg:col-span-5 p-10 border-none bg-white/50 shadow-sm">
					...
					<p className="text-lg text-slate-500 leading-relaxed italic">"We release limited capsules instead of trends. Each drop is developed with circularity in mind."</p>
					...
					<div className="rounded-[2rem] bg-slate-900 p-8 text-white shadow-xl">
						...
						<p className="mt-4 text-sm leading-relaxed text-slate-400">Our teams collaborate digitally to keep iteration fast while production remains intentionally slow.</p>
					</div>
				</motion.div>

				{/* Right Column: Manifesto Staggered List */}
				<motion.div variants={staggerContainer} whileInView="animate" viewport={{ once: true }} className="lg:col-span-7 space-y-6">
					<p className="tag-pill mb-8">Manifesto</p>
					{atelierHighlights.map((item, index) => (
						<motion.div key={item.title} variants={fadeInUp} className="group rounded-3xl border border-slate-100 bg-white p-8 transition-all hover:shadow-lg hover:-translate-y-1">
			...
			<motion.section initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-panel relative overflow-hidden bg-slate-50 py-20 px-12 border-none">
				<div className="relative z-10 grid gap-12 sm:grid-cols-3">
					{[
						...
						<div key={i} className="space-y-4">
							...
							<p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400 max-w-[200px]">{metric.label}</p>
						</div>
					))}
				</div>
			</motion.section>
